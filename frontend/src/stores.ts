...
const storeVersion: number = JSON.parse(String(localStorage.getItem("version")));
if (!storeVersion) {
  const settings = JSON.parse(String(localStorage.getItem("settings"))) as Settings;
  localStorage.setItem("settings", JSON.stringify(settings ?? defaultSettings));

  const presets = JSON.parse(String(localStorage.getItem("presets"))) as Preset[];
  localStorage.setItem("presets", JSON.stringify(defaultPresets.map((preset, index) => {
    const system = presets?.at(index)?.system ?? preset.system; // HINT: system config from no version
    return { ...preset, system };
  })));
  localStorage.setItem("version", JSON.stringify(1));
};

export const settings = writable<Settings>(JSON.parse(String(localStorage.getItem("settings"))));
...
export const presets = writable<Preset[]>(JSON.parse(String(localStorage.getItem("presets"))));
presets.subscribe(presets => localStorage.setItem("presets", JSON.stringify(presets)));

export const fallbackPreset = { name: "?", system: "?", image: imageNeutral, enabled: true }